...
CONF_FILE="arch.conf"
GLOBALS_FILE="arch-globals.conf"
LOG_FILE="arch.log"
ASCIINEMA_FILE="arch.asciinema"
...
function create_user_homectl() {
    ...
    ### something missing, inside arch this not works, after install the user is in state infixated
...
function bootloader_grub() {
    ...
    echo "# arch" >> /mnt/etc/default/grub
...
function desktop_environment_kde() {
    pacman_install "plasma-kde-desktop"
...
function packages() {
    ...
    if [ -n "$AUR" -o -n "$AUR_PACKAGES" ]; then
        packages_aur
    fi
}
    ...
    if [ "$REBOOT" == "true" ]; then
        ...
        if [ "$ASCIINEMA" == "false" ]; then
            ...
            for (( i = 15; i >= 1; i-- )); do
                ...
                if [ $? -eq 0 ]; then
                    echo ""
                    echo "Restart aborted. You will must do a explicit reboot (./arch-reboot.sh)."
                    echo ""
                    REBOOT="false"
                    break
                fi
            ...
            echo ""
            echo "Restart aborted. You will must terminate asciinema recording and do a explicit reboot (exit, ./arch-reboot.sh)."
            echo ""
            REBOOT="false"
        fi
        ...
        if [ "$ASCIINEMA" == "false" ]; then
            echo ""
            echo "You will must do a explicit reboot (./arch-reboot.sh)."
            echo ""
        else
            echo ""
            echo "You will must terminate asciinema recording and do a explicit reboot (exit, ./arch-reboot.sh)."
            echo ""
        fi
    fi
}
